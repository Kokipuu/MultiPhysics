...
!	  open(600,file='test11_result/DDL-test.dat')
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc!Pt粒子並べる処理
      stdist = ptkosi3
      do 100 k=1,n1z_pt
...
ccccccccccccccccccccccccccccccccccccccccccccccc!Ar粒子の初期配置
	   ...
	   arofstz1 = underz*2.0D0 + dble(n1z_pt)*ptkosi3/2.00D0
...
900      continue
        ...
        write(8,'(4D15.7)')totene_ar, totpot_ar, totkin_ar, temp_ar
		
		totene = totene_pt + totene_ar
		totpot = totpot_pt + totpot_ar
		totkin = totkin_pt + totkin_ar
		
		write(9,'(4D15.7)')totene,totpot,totkin
        return
        end
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
2     continue
      ...
      ddl = ddl / dble(10)
      write(60,'(3E15.7)')dens_ar,dens_pt,ddl
      return
	  end
...
!			if(mod(nowstp,1000).eq.0)then
!			  write(500,'(2D15.7)')q_bot,q_pt(i)
!			endif
51      continue

         q_bot = tot_q_bot*1.00D4/(xsyul0*ysyul0)*(-1.0D0)
